...
public class Player : Entity
{
    ...
    protected override void Start()
    {
        ...
        indicatorManager = transform.GetComponentInChildren<IndicatorManager>();
                ...
                if (skillIndex >= 0)
                {
                    ...
                    if (currentSkill >= 0)
                    {
                        ...
                        if (!_currentSkill.canCancelDuringCasting || currentSkill == skillIndex)
                        {
                            ...
                            if(indicatorManager) 
                                indicatorManager.showSkillRange(false);
                            return;
                        }
                        else
                        {
                    ...
                    if (left && 0 <= wantSkill)
                    {
                        ...
                        if (!CorrectedTarget(skill, entity))
                        {
                                ...
                                (wantSkill >= skills.Count && UI_Toggle.self.itemsSmartCasting[wantSkill - skills.Count]))
                            {
                                ...
                                if(indicatorManager) indicatorManager.showSkillRange(false);
                            }
                            return;
                        }
    ...
    {
      //  Debug.Log("KeyInputHandling Call");
        ...
        for (int i = 0; i < UI_Toggle.self.skillsHotkey.Length; i++)
        {
            if (Input.GetKeyDown(UI_Toggle.self.skillsHotkey[i]))
            {
                ...
                else if (skill.showSelector && !UI_Toggle.self.skillsSmartCasting[wantSkill])
                {
                    ...
                    if (indicatorManager)
                    {
                       // Debug.Log("Indicator Manager Call : " + skill.template);

                        indicatorManager.showSkillRange(true, skill, skillCastRange);

                        indicatorManager.UpdateSkillIndicator(skill, cursor.transform.position, skillCastRange);
                    }
                ...
                else if (skill.showSelector && !UI_Toggle.self.itemsSmartCasting[i])
                {
                    if (indicatorManager)
                    {
                      //  Debug.Log("Indicator Manager Call : " + skill.template);
                        ...
                        indicatorManager.UpdateItemSkillIndicator(skill, cursor.transform.position, skillCastRange);
                    }
